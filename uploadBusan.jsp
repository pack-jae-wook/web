   ...
   <div class="container">
      <table class="table table-striped" style="text-align: center; border:1px sloid #dddddd">
         ...
         <%
            ...
            ArrayList<Uploadfile> list = uploadDAO.getList(pageNumber);
            for(int i = 0; i < list.size(); i++){
         %>
            <tr>
               <td><%= list.get(i).getLoadtourismID() %></td>
               <td><a href="uploadView.jsp?loadtourismID=<%= list.get(i).getLoadtourismID() %>"><%= list.get(i).getLoadtourismTitle() %></td>
               <td><%= list.get(i).getuserID() %></td>
               <td><%= list.get(i).getLoadtourismDate().substring(0,11)+list.get(i).getLoadtourismDate().substring(11,13)+"시"+list.get(i).getLoadtourismDate().substring(14,16)+"분" %></td>
      ...
      <%
         if(pageNumber !=1){
      %>
      <a href="uploadBusan.jsp?pageNumber=<%=pageNumber - 1%>" class="btn btn-success btn-arraw-left">이전</a>
      <%
         }if(uploadDAO.nextPage(pageNumber+1)) {
      %>
      <a href="uploadBusan.jsp?pageNumber=<%=pageNumber + 1%>" class="btn btn-success btn-arraw-left">다음</a>
      <%
         }
      %>

      <a href="uploadWrite.jsp" class="btn btn-primary pull-right">글쓰기 </a>
   </div>
